...
function PMSpamPanel(props: {Server: any, player: any;}){
    ...
    function handleMessageEdit() {
        ...
        value.forEach((message:string) => {
            const index = newMessageList.indexOf(message);
            if (index > -1) 
                newMessageList[index] = editingMessage;
        })
        setMessageList(newMessageList)
        setValue([])
        setEditingMessage("")
            ...
            <div className='pmspampanel-container'>
                ...
                <FormControl className='pmspampanel-form'>
                    ...
                    <div className='pmspampanel-form-editing'>
                        <input type="text" onChange={handleNewMessageChange} placeholder="Toe te voegen bericht..." value={newMessage} />
                        <Button style={{marginLeft: 10, width: 150}} onClick={handleAddClick} variant="contained" startIcon={<AddIcon />}>
                            Voeg toe
                        </Button>
                        {canEdit && 
                            <Menu className='item-contextmenu' menuButton={
                                
                                <Button style={{marginLeft: 10, width: 150}} onClick={handleBewerkClick} variant="contained" startIcon={<EditIcon />}>
                                    Bewerk
                                </Button>
                            }>
                                <MenuHeader>Message Editor</MenuHeader>
                                <MenuItem className='item-context-button' onClick={function(e: any) {e.keepOpen = true;}} style={{width: "400px"}} ><input type="text" style={{width: "100%"}} onChange={handleEditingMessage} placeholder="Geef een message in..." value={editingMessage} /></MenuItem>
                                <MenuItem className='item-context-button' onClick={function(e: any) {e.keepOpen = true;}} style={{width: "400px"}} >
                                    <Button onClick={handleMessageEdit} style={{width: "100%"}} variant="contained" startIcon={<EditIcon />}>
                                        Pas bericht aan
                                    </Button>
                                </MenuItem>
                            </Menu>
                        ...
                        <Button style={{marginLeft: 10, width: 150}} onClick={handleRemoveClick} variant="contained" startIcon={<DeleteIcon />}>
                            Verwijder
                        </Button> 
                    </div>
                    <Button 
                        variant="outlined"
                        sx={{ width: "100%", margin: "10px 0"} } onClick={() => startPlayerSpamming(true)}
                    >
                        Start Spamming
                    </Button>
                    <Button 
                        variant="outlined"
                        sx={{ width: "100%"} } onClick={() => startPlayerSpamming(false)}
                    >
                        Stop Spamming
                    </Button>
                </FormControl>
