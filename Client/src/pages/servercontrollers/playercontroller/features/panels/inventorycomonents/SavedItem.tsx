      ...
      if (props.item === null) {
            ...
            displayname = (
              <>
                ...
                <span style={{ color: "rgb(200, 200, 200)" }}>
                  {stripColor(itemstack.itemmeta.displayname)}
                </span>
              </>
            );
          }
        ...
        var type = (
          <>
            ...
            <span style={{ color: "rgb(200, 200, 200)" }}>
              {itemstack.type.replaceAll("_", " ").toString().toLowerCase()}
            </span>
          </>
        );
        var lore = [];
    ...
    return (
        ...
        menuButton={
          <Tooltip placement="top" title={tooltip} disableInteractive>
            <div className="slot noselect">
              {itemstack.itemmeta !== undefined &&
              itemstack.itemmeta.enchants !== undefined ? (
                <Enchanting />
              ) : (
                <></>
              )}
      ...
      >
        ...
        <MenuItem
          ...
          onClick={() => props.handleItemClick("give", props.item)}
        >
          <FaLocationArrow />
          <span>Give Item</span>
        </MenuItem>
        <MenuItem
          className="slot-context-button"
          onClick={() => props.handleItemClick("edit", props.item)}
        >
          <FaEdit />
          <span>Edit item</span>
        </MenuItem>
        <MenuItem
          className="slot-context-button"
          onClick={() => props.handleItemClick("remove", props.item)}
        >
          <FaTrash />
