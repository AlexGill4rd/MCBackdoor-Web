...
function ArmorPanel(props: { server: IServer; player: IPlayer | null }) {
      ...
      socket.on(`player:get-armor-${props.player?.uuid}`, (armorData) => {
        ...
        activeArmor.forEach((item: any) => {
          if (item.Empty) {
            ...
            if (
              typeof item.ItemstackJson === "object" &&
              !Array.isArray(item.ItemstackJson) &&
              item.ItemstackJson !== null
            )
              itemstackJSON = item.ItemstackJson;
            else itemstackJSON = JSON.parse(item.ItemstackJson);
            var itemType = "minecraft:" + itemstackJSON.type.toLowerCase();
            var icon: string = items.filter(function (value) {
              return value.id === itemType;
            })[0].texture;
  ...
  function handleSkinDownload() {
    if (props.player === null) return;
    forceDownload(
      "https://crafatar.com/skins/" + props.player.uuid + ".png",
      props.player.displayname + "_skin_download.png"
    );
    socket.emit(
      "feature:player-log",
      socket.id,
      "Je hebt de skin van deze speler gedownload!",
      "success"
    );
  }
    ...
    <>
      ...
      <div className="armor-customizer-container">
        ...
        <div className="armor-customizer-container-right">
          <span className="armor-customizer-container-right-header">
            Slots:
          </span>
          <div className="armor-customizer-container-right-slots">
            {armorItems.map((item: any) => {
              var type = "";
              if (item.Slot === 36) type = "Boots";
              else if (item.Slot === 37) type = "Leggings";
              else if (item.Slot === 38) type = "Chestplate";
              else if (item.Slot === 39) type = "Helmet";
              else if (item.Slot === 40) type = "Offhand";
              if (item.Empty) {
                return (
                  ...
                  <div className="armor-customizer-slot-container">
                    <Menu
                      ...
                      menuButton={
                        <div className="armor-customizer-slot-container-item">
                          {item.ItemstackJson.itemmeta !== undefined &&
                          item.ItemstackJson.itemmeta.enchants !== undefined ? (
                            <Enchanting />
                          ) : (
                            <></>
                          )}
                          {!item.Empty ? (
                            <img src={item.ItemstackJson.texture} />
                          ) : (
                            <CircularProgress />
                          )}
                        </div>
                    ...
                    >
                      ...
                      <MenuItem
                        ...
                        onClick={() => handleUnequipClick(item)}
                      >
                        Unequip Item
                      </MenuItem>
                      <MenuItem
                      ...
                      <SubMenu label="Item Info">
                        {item.ItemstackJson.itemmeta !== undefined &&
                        item.ItemstackJson.itemmeta.displayname !==
                          undefined ? (
                        ...
                        {item.ItemstackJson.itemmeta !== undefined &&
                        item.ItemstackJson.itemmeta.lore !== undefined ? (
                          <MenuItem className="info">
                            <span>Lore:</span>
                            {item.ItemstackJson.itemmeta.lore.map(
                              (line: string, index: number) => {
                                return (
                                  <span className="info-text" key={index}>
                                    {"- " + line}
                                  </span>
                                );
                              }
                        ...
                        item.ItemstackJson.itemmeta.enchants !== undefined ? (
                          <MenuItem className="info">
                            ...
                            {item.ItemstackJson.itemmeta.enchants.map(
                              (enchant: string, index: number) => {
                                return (
                                  <span className="info-text" key={index}>
                                    {"- " + enchant}
                                  </span>
